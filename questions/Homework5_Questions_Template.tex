...
The difference between the construction of convolutional filters in classical
machine learning techniques and those used in convolutional neural networks is
that the filter kernels in CNN's are learned. The weights in the kernels in
classical techniques are designed by hand with a human-designed purpose in mind: to
mimic gaussians or detect edges, for example. In addition, in a CNN convolution
typically refers to correlation (where the kernel is not rotated).
...
\begin{tcolorbox}[colback=white!5!white,colframe=green!75!black]
    \setbox0=\hbox{\parbox[t]{\textwidth}{
    %%%%%%% ANSWER STARTS HERE %%%%%%%%%%%%%%%%%%%%%%%%%%%%
An advantage to human-designed kernels is that they are well-understood by
human researchers. Researchers know what makes certain kernels effective for
specific tasks, which gives a system that uses them a degree of
interpretability by default.

Another advantage to human-designed kernels is that they can be added to a model
quickly without additional training. If we would like to learn the weights for a
kernel, that takes additional time, while human-designed kernels can be used out
of the box.

An advantage of learned kernels is that they can be tweaked to work well for a
specific task. An effective machine learning method can pick values for kernel
that takes advantage of patterns in the data that humans don't recognize.

Another disadvantage of human-designed kernels is that there just aren't that
many of them. Humans can design kernels for a few specific tasks, but cannot
feasibly design the number needed for deeply nesting kernels like in a CNN.
    %%%%%% Remove this line in your answer! %%%%%%
...
With a fixed number of nodes in the network, a dense multi-layer perceptron will
take more time to train. There are simply more weights to be optimized. This can
mean that a final model produced this this way would be poorly optimized if
there is a limited budget of training time. In addition, making a network more
dense has diminishing benefits and may not have that large of an improvement in
performance. A dense network could take longer to evaluate and require more
memory in order to perform the additional calculations. A third difference is
that dense multi-layer perceptron may have better performance on training since
it can fit more of the details of the model.
...
A larger learning rate can make training less predictable as it takes larger
steps in the feature space. This can lead to it oscillating back and forth over
a bowl-region in the feature space. A smaller learning rate can have the
downside of being more likely to get caught in local minima, which can decrease
...
Having a larger batch-size can make training slower. However, a small batch size
has the downside that a small batch may be less representative of the true data,
and so a model produced with a small batch size may perform worse. In addition,
training with a small batch size can be unstable.
    %%%%%% Remove this line in your answer! %%%%%%
...
With more epochs, the model is almost certain to better fit to the training
data. However, the downside of an increased number of epochs is that the model
may learn to overfit on patterns in the data that reflect noise in the data,
which can lead to worse test performance. Some models may become additionally
unstable after a large number of epochs of training.
...
\pagebreak
    ...
    Provides local scale invariance & $\blacksquare$ \\
...
Max pooling does preserve translation equivariance. The pooling process samples
from a neighbohood of pixels in order to determine the activation in the new
layer. Since the max pool takes the maximum of the activations in the previous
layer, shifts that take place on the order of the window size will not be
observed. This is a special property of the max function, since it is naturally
anti-aliasing. The network will be equivariant to shifts larger than the window
size since they will lead to activations across other windows. 
...
One factor that would make me more confident in its use would be if my model's
test accuracy and training accuracy were both comparably high. This would make
me more likely to think that the model was picking up on true patterns within
the data instead of fitting on noise or irrelevant details.

Another factor that would make me more confident in its use is if Rhode Island
Hospital did not have access to good prior detection technology and their human
doctors performed poorly at detecting these images. If Rhode Island Hospital had
no other better options, then a deploying a poorly understood machine learning
model would be undesirable but would might be warranted.

A factor that would make be less confident in its use is if the International
Skin Imaging Collaboration dataset was not representative of the kinds of images
that Rhode Island Hospital would capture clinically. If my model was being
deployed outside of its training environment, then its behavior could be
unpredictable and create harm due to false positive or false negative diagnoses.
...
The LIME techinique would be effective in understanding the predictions of
the model because it communicates which features in the image were integral in
making the decision. For example, it can provide a view of the features in the
image that were important in making the decision, since these superpixels being
turned on an off show the change in the confidence of the decision.

The saliancy map could also help with understanding the prediction because it
gives a view into which features of the image had the biggest impact on the
final outcome. If the saliancy map showed a high activation around features of
the image, like rulers, it may cast some doubt into the decision.

Both the LIME maps and the saliancy maps could be shown to a patient, who could
understand that they represented the features in the lesion that the model
thought were most significant for the diagnosis. This could be comparable to a
human doctor describing the features of the lesion that they thought were
alarming.
